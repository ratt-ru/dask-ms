...
@pytest.mark.parametrize(
    "select_cols", [["STATE_ID", "TIME", "DATA"]], ids=select_cols_str
)
@pytest.mark.parametrize(
    "shapes", [{"row": 10, "chan": 16, "corr": 4}], ids=lambda s: f"shapes={s}"
)
@pytest.mark.parametrize(
    "chunks",
    [
...
def test_dataset(ms, select_cols, group_cols, index_cols, shapes, chunks):
    ...
    echunks = {
        "chan": normalize_chunks(chunks.get("chan", chans), shape=(chans,))[0],
...
def test_dataset_updates(ms, select_cols, group_cols, index_cols, shapes, chunks):
    """Test dataset writes"""
...
def test_dataset_assign(ms):
    ...
    if have_xarray:
        ...
        match = (
            "Existing dimension size 9 for dimension 'row' "
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            object,
            marks=pytest.mark.xfail(reason="putcol can't handle " "lists of ints"),
        ),
        pytest.param(
            np.uint16,
            marks=pytest.mark.xfail(
                reason="RuntimeError: RecordRep::"
                "createDataField: unknown data"
                " type 17"
            ),
        ),
        pytest.param(np.uint8, marks=pytest.mark.xfail(reason="Creates uint16 column")),
    ],
)
def test_dataset_add_column(ms, dtype):
    ...
    writes = write_datasets(
        ms, nds, ["BITFLAG"], descriptor="ratt_ms", column_keywords=col_kw
    )
...
@pytest.mark.parametrize("chunks", [{"row": (36,)}, {"row": (18, 18)}])
def test_dataset_multidim_string_column(tmp_path, chunks):
    ...
    datasets = read_datasets(table_name, [], [], [], chunks={"row": chunks["row"]})
    assert len(datasets) == 1
...
def test_dataset_create_table(tmp_path, dataset_chunks, dtype):
    ...
    freq = da.linspace(0.856e9, 2 * 0.856e9, 64, chunks=16)
...
def test_write_dict_data(tmp_path, chunks, dtype):
    ...
    def _vis_factory(chan, corr):
        ...
        return rs.normal(size=(1, nchan, corr)) + rs.normal(size=(1, nchan, corr)) * 1j
    ...
    layer = {
        k: {"r%d" % (i + 1): _vis_factory(chan, corr) for i in range(r)}
    ...
    table_name = os.path.join(str(tmp_path), "test.table")
    writes, table_proxy = write_datasets(
        table_name,
        ds,
        ["DATA"],
        table_proxy=True,
